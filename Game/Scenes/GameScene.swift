...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func setUpMoveButtons() {
        let buttonLeft = ButtonNode(position: CGPoint(x: size.width * 0.13, y: size.width * 0.15), zPosition: 21, name: "leftButton", action: playerMoveLeft)
        let buttonRight = ButtonNode(position: CGPoint(x: size.width * 0.20, y: size.width * 0.15), zPosition: 21, name: "rightButton", action: playerMoveRight)
